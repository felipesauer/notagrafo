---
mnema:
  key: NOTA-EPIC-9
  kind: epic
  state: OPEN
  title: Auditoria 2 — Cobertura 90% e alinhamento de contrato
  description: >-
    Achados da 2ª auditoria (dupla-checada): elevar cobertura unit para 90%,
    alinhar status/progresso do job ao contrato, consistência de 404 em
    /eventos, cobrir filtros do GET /nf, e2e dos fluxos novos, decidir sobre
    historicoPrecoProduto e dívida do export em memória. Baseline verde (unit
    37/37, integração 66/66).
  metadata: {}
  created_at: '2026-06-28T22:03:36.608Z'
  closed_at: null
---
# Auditoria 2 — Cobertura 90% e alinhamento de contrato
