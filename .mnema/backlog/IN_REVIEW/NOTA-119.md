...
mnema:
  ...
  description: >-
    ...
    topbar com busca/Cmd+K/tema/idioma (busca centralizada), e coluna de
    Insights colapsável à direita (moldura, populada na Fase 2). Ajustar
    router.tsx: / vira Home/Overview, Explorer vai para /explorar (entidades via
    ?entity=). Atualizar CommandPalette.tsx e i18n/pt-BR.ts (bloco sidebar.*)
    para a nova IA. Manter Cmd+K, saved views (useSavedViews) e peek. Preservar
    data-testid app-sidebar. Referência visual: protótipo
    scratchpad/notagrafo-prototype.html.
  acceptance_criteria:
    - / renderiza a Home; /explorar renderiza o Explorer com troca de entidade
    - >-
      sidebar de ícones + topbar (busca central) + Insights panel colapsável
      (persiste estado)
