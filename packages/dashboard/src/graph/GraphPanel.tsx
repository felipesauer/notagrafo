                    ...
                    {node.cnpj && (
                        <div className="mt-1 grid gap-1">
                            <Button asChild variant="outline" size="sm">
                                <Link to={'/' as string} search={{ entity: 'notas', cnpjEmitente: node.cnpj } as never}>{t('nf.detalheTitulo')}</Link>
                            </Button>
                            <Button asChild variant="outline" size="sm">
                                <Link to={'/' as string} search={{ entity: 'notas', cnpjEmitente: node.cnpj, comImposto: true } as never}>{t('grafo.nfsComImposto')}</Link>
                            </Button>
                        </div>
                    )}
                </dl>
                ...
                <dl className="grid gap-3">
                    ...
                    {details.ncm ? (
                        <Button asChild variant="outline" size="sm" className="mt-1">
                            <Link to={'/' as string} search={{ entity: 'notas', ncm: String(details.ncm) } as never}>{t('grafo.nfsDoNcm')}</Link>
                        </Button>
                    ) : null}
                </dl>
            )}
