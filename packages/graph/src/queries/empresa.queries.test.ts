...
import { getEmpresaStats, getEmpresaGrafo, DepthForaDoLimiteError } from './empresa.queries.js';

describe('getEmpresaStats (unit)', () => {
    it('agrega contagens e valores do record', async () => {
        ...
        const stats = await getEmpresaStats(driver, '111');
    ...
    it('sem record → zeros e datas null', async () => {
        ...
        const stats = await getEmpresaStats(driver, 'x');
        expect(stats.totalNFsEmitidas).toBe(0);
        expect(stats.primeiraEmissao).toBeNull();
    });
});

describe('getEmpresaGrafo (unit)', () => {
    it('rejeita depth fora de [1,4]', async () => {
        const { driver } = makeFakeDriver(() => []);
        await expect(getEmpresaGrafo(driver, '111', { depth: 5 })).rejects.toThrow(DepthForaDoLimiteError);
        await expect(getEmpresaGrafo(driver, '111', { depth: 0 })).rejects.toThrow(DepthForaDoLimiteError);
    });
    ...
    it('mapeia nós (1ª query) e arestas (2ª query); grau mínimo 1', async () => {
        ...
        const grafo = await getEmpresaGrafo(driver, '111', { depth: 2, direction: 'both', limit: 50 });
    ...
    it('direction destinatario muda o padrão de relação', async () => {
        ...
        await getEmpresaGrafo(driver, '111', { direction: 'destinatario', depth: 1 });
