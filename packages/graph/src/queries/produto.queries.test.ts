...
describe('topProdutos (unit)', () => {
    ...
    it('metrica quantidade ordena por quantidadeTotal e aplica filtros ncm/data', async () => {
        ...
        await topProdutos(driver, { metrica: 'quantidade', ncm: '6109', dataInicio: '2026-01-01', dataFim: '2026-12-31' });
...
describe('historicoPrecoProduto (unit)', () => {
    it('calcula preço médio por período e ordena ascendente', async () => {
        ...
        const { driver, runs } = makeFakeDriver(() => rows);
        const hist = await historicoPrecoProduto(driver, 'p1');
    ...
    it('produto sem histórico → []', async () => {
        ...
        expect(await historicoPrecoProduto(driver, 'nope')).toEqual([]);
    });
});
